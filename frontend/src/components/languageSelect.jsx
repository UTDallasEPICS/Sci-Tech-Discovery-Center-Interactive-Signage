...
const LangaugeSelect = () => {
  ...
  const handleLanguageSelect = (buttonCode) => {
    if (chosen) return; // prevent double-tap
    setChosen(true);
    fetch(`/api/receive-button/?button=${buttonCode}`)
      .catch((err) => console.error("Error sending button press:", err));
  };
    ...
    <div className="w-full h-screen grid grid-rows-12 justify-evenly bg-lang-bg bg-contain">
      ...
      <div className="flex justify-evenly h-full w-full row-start-6 row-end-8">
        <Button
          ...
          onClick={() => handleLanguageSelect("a")}
          disabled={chosen}
        />
        <Button
          mainText={"Español"}
          subText={"Spanish"}
          color="bg-orange"
          onClick={() => handleLanguageSelect("b")}
          disabled={chosen}
        />
        <Button
          mainText={"ASL"}
